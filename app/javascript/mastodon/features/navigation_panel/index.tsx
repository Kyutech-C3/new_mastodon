...
import { c3_official_site_url, c3_toybox_url, mascot, timelinePreview, trendsEnabled, me } from 'mastodon/initial_state';
        ...
        {signedIn && (
          <>
            ...
            {(c3_official_site_url || c3_toybox_url) && (
              <>
                {/* 公式サイトのURLがあればリンクを表示 */}
                {c3_official_site_url && <ColumnLink transparent href={c3_official_site_url} target='_blank' rel='noreferrer noopener' icon='laptop' iconComponent={ComputerIcon} text={intl.formatMessage(messages.officialSite)} />}
                {/* ToyBoxのURLがあればリンクを表示 */}
                {c3_toybox_url && <ColumnLink transparent href={c3_toybox_url} target='_blank' rel='noreferrer noopener' icon='archive' iconComponent={InventoryIcon} text={intl.formatMessage(messages.toybox)} />}
                <hr />
              </>
            )}
